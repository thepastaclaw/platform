...
use crate::{
    DashSDKError, DashSDKErrorCode, DashSDKPutSettings, DashSDKResult,
    DashSDKStateTransitionCreationOptions, DashSDKTokenFreezeParams, FFIError, SDKHandle,
    SignerHandle,
};
...
#[no_mangle]
pub unsafe extern "C" fn dash_sdk_token_unfreeze(
    ...
    match result {
        Ok(_unfreeze_result) => DashSDKResult::success(std::ptr::null_mut()),
...
mod tests {
    ...
    fn create_valid_unfreeze_params() -> DashSDKTokenFreezeParams {
    ...
    unsafe fn cleanup_unfreeze_params(params: &DashSDKTokenFreezeParams) {
    ...
    #[test]
    fn test_unfreeze_with_null_sdk_handle() {
        let transition_owner_id = create_valid_transition_owner_id();
        let params = create_valid_unfreeze_params();
        ...
        unsafe {
            cleanup_unfreeze_params(&params);
        }
    }

    #[test]
    fn test_unfreeze_with_null_transition_owner_id() {
    ...
    #[test]
    fn test_unfreeze_with_null_params() {
        let sdk_handle = create_mock_sdk_handle();
        let transition_owner_id = create_valid_transition_owner_id();
        let identity_public_key = create_mock_identity_public_key();
        let signer = create_mock_signer();
    ...
    #[test]
    fn test_unfreeze_with_null_identity_public_key() {
        let sdk_handle = create_mock_sdk_handle();
        let transition_owner_id = create_valid_transition_owner_id();
        let params = create_valid_unfreeze_params();
        let signer = create_mock_signer();
    ...
    #[test]
    fn test_unfreeze_with_null_signer() {
        let sdk_handle = create_mock_sdk_handle();
        let transition_owner_id = create_valid_transition_owner_id();
        let params = create_valid_unfreeze_params();
        let identity_public_key = create_mock_identity_public_key();
    ...
    #[test]
    fn test_unfreeze_with_null_target_identity_id() {
        let sdk_handle = create_mock_sdk_handle();
        let transition_owner_id = create_valid_transition_owner_id();
        let mut params = create_valid_unfreeze_params();
    ...
    #[test]
    fn test_unfreeze_with_public_note() {
        let transition_owner_id = create_valid_transition_owner_id();
        let mut params = create_valid_unfreeze_params();
        params.public_note = CString::new("Unfreezing account after verification")
            .unwrap()
            .into_raw();
    ...
    #[test]
    fn test_unfreeze_with_serialized_contract() {
        let transition_owner_id = create_valid_transition_owner_id();
        let mut params = create_valid_unfreeze_params();
    ...
    #[test]
    fn test_unfreeze_with_different_token_positions() {
        let transition_owner_id = create_valid_transition_owner_id();
